module DebugLogging
  module Util
    ...
    def extract_payload_and_config(methods_to_log, config = nil, payload = nil)
      ...
      scoped_payload = (methods_to_log.is_a?(Array) && methods_to_log.last.is_a?(Hash) && methods_to_log.pop.dup) || {}
      if payload
    ...
    def debug_logged_proc(original_method, method_to_log, pointer_ref)
      ->(payload, config_opts) do
        define_method(method_to_log) do |*args, &block|
          config_proxy = if (proxy = instance_variable_get(DebugLogging::Configuration.config_pointer(pointer_ref,
                                                                                                      method_to_log)))
                           proxy
                         else
                           ...
                           instance_variable_set(DebugLogging::Configuration.config_pointer(pointer_ref, method_to_log),
                                                 proxy)
                           proxy
                         end
    ...
    module_function :debug_logged_proc
  end
end