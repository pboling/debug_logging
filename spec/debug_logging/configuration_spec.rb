  ...
  context "config" do
      ...
      context "with block" do
        context "instance logging" do
          ...
          it "has correct return value" do
            ...
            expect(instance_logged_klass_explicit.new.i_with_ssplat(
              "a",
              1,
              true,
              ["b", 2, false],
        ...
        context "instance notification" do
          ...
          it "notified" do
            output = capture("stdout") do
              ...
              instance_notified_klass_explicit.new(
                action: "Update",
                id: 1,
                msg: {greeting: "hi"},
      ...
      context "inheritance" do
        ...
        it "uses separate configs" do
          ...
          output = capture("stdout") do
            ...
            expect(child_singleton_logged_and_notified_klass.perform(
              "r",
              4,
              true,
              ["u", 2, false],
        ...
        it "can override configs" do
          ...
          expect(output).to match(/DEBUG -- : ChildSingletonLoggedArgsKlass::snakes\(\["abcdefghijklmnopqrstuvwxy<><><>\) #{enter_tail_match_src}$/)
        end
      end
    ...
    context "per method config" do
      ...
      context "instance logging" do
        ...
        it "uses separate configs" do
          ...
          expect(output).to match(/#i\(\) #{inv_id_regex}/)
          expect(output).not_to match(/#i #{bench_tail_inv_match_src}/)
          # expect(output).to match(/#i_with_ssplat\("a", 1, true, \["b", 2, false\], {:c=>:d, :e=>:f}\)/)
          # expect(output).to match(/#i_with_ssplat #{bench_tail_match_src}/)
          # expect(output).to match(/#i_with_dsplat\(\*\*{:a=>"a", :b=>1, :c=>true, :d=>\["b", 2, false\], :e=>{:c=>:d, :e=>:f}}\)/)
          # expect(output).not_to match(/#i_with_dsplat #{bench_tail_inv_match_src}/)
          # expect(output).to match(/#i_with_ssplat\("z", 1, true, \["y", 2, false\], {:c=>:d, :e=>:f}\) #{inv_id_regex}/)
          # expect(output).not_to match(/#i_with_ssplat #{bench_tail_inv_match_src}/)
        end
        ...
        it "has correct return value" do
          ...
          expect(instance_logged_klass_explicit.new.i_with_dsplat(
            a: "a",
            b: 1,
            c: true,
            d: ["b", 2, false],
    ...
    context "last_hash_to_s_proc" do
      context "class level config" do
        ...
        it "logs" do
          ...
          expect(output).to match(/::#{cw_src("k_with_dsplat")}\(\[:a, :b, :c, :d, :e\]\) ~/)
        end
    ...
    context "multiple_last_hashes" do
      context "class level config" do
        ...
        it "logs" do
          output = capture("stdout") do
            singleton_logged_klass.k_with_ssplat(
              "a",
              1,
              true,
              ["b", 2, false],
          ...
          expect(output).to match(/::#{cw_src("k_with_ssplat")}\("a", 1, true, \["b", 2, false\], \[:c, :e\], \[:a, :b, :c, :d, :e\]\) ~/)
          expect(output).to match(/::#{cw_src("k_with_dsplat")}\(\[:a, :b, :c, :d, :e\]\) ~/)
        end

        it "has correct return value" do
          expect(singleton_logged_klass.k_with_ssplat(
            "a",
            1,
            true,
            ["b", 2, false],
      ...
      context "instance level config" do
        ...
        it "logs" do
          output = capture("stdout") do
            instance_logged_klass_dynamic.new.i_with_ssplat(
              "a",
              1,
              true,
              ["b", 2, false],
    ...
    context "last_hash_max_length" do
      context "when last_hash_to_s_proc is set" do
        ...
        it "logs ellipsis" do
          ...
          expect(output).to match(/::#{cw_src("k_with_dsplat")}\(\[:a, ✂️ …\) ~/)
        end
    ...
    context "args_max_length" do
      context "when last_hash_to_s_proc is set" do
        ...
        it "logs full messages when under max" do
          ...
          expect(output).to match(/SingletonLoggedKlass::#{cw_src("k_with_ssplat")}\("a", 1, true, \[:c, :e\]\) ~/)
        end
        ...
        it "logs ellipsis" do
          ...
          expect(output).to match(/SingletonLoggedKlass::#{cw_src("k_with_ssplat")}\("a", 1, true, \["b", 2 ✂️ …, \[:c, :e\]\) ~/)
        end
      ...
      context "when last_hash_to_s_proc is not set" do
        ...
        it "logs ellipsis" do
          ...
          expect(output).to match(/::#{cw_src("k_with_ssplat")}\("a", 1, true, \["b", 2 ✂️ …\) ~/)
        end
